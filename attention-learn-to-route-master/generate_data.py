...
def generate_tsptw_data(dataset_size, tsptw_size):
    loc = np.random.uniform(size=(dataset_size, tsptw_size, 2))
    tw_center = np.random.uniform(size=(dataset_size, tsptw_size, 1))
    tw_width = np.random.uniform(size=(dataset_size, tsptw_size, 1), low=1e-3, high=0.5)
    tw_lb = np.array(tw_center - tw_width)
    tw_lb[tw_lb < 0] = 0
        ...
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    dataset = generate_tsptw_data(opts.dataset_size, graph_size)
                elif problem == 'vrp':
